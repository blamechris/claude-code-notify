...
**Labels each sub-issue will receive:** enhancement, from-review
...
```bash
# Default sub-issue labels: enhancement or from-review
...
## Decomposition Guidance for claude-code-notify

Decomposition is rare in this small codebase (two scripts, ~380 lines total). Most issues are directly actionable in a single PR. Only decompose if:

- The change spans both `notify.sh` and the hook event handler dispatch, OR
- A new persistent-state mechanism is introduced that needs schema + reader + writer split apart.

Natural seams when decomposition is justified:

- One sub-issue per hook-event type (idle, permission, subagent, bg-bash, heartbeat)
- Split config/state/transport: env-var > .env > defaults hierarchy → state files in `/tmp` and `~/.claude-notify` → Discord webhook call
