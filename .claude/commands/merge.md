...
- `$ARGUMENTS` - PR numbers, `all`, or flags:
  ...
  - `--no-test` — skip post-merge test run
  - `--test-only` — run post-merge tests on current main without merging
...
#### Step 3b: Run test suite

```bash
bash test/run-tests.sh
```

If tests fail, report error and suggest manual intervention.
...
| Post-merge test failure | Report error, suggest manual intervention |
...
7. **Shell safety first** — all post-merge scripts must use `set -euo pipefail` and proper quoting
