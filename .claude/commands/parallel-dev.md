...
When the queue contains issues that span multiple hook-event types or introduce new persistent-state mechanisms:
...
This is a Bash project with no package manager install step.
...
Run the shell test suite to confirm tests fail:
...
Scope examples: `notify`, `hooks`, `config`, `state`.
