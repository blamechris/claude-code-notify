...
When the queue contains issues labeled with high complexity or equivalent:
...
This is a Bash project with no package manager install step. Dependencies (jq, curl, bash) are system tools.
...
Tests are shell-based. Run the test suite to confirm tests fail.
...
Verify shell safety: `set -euo pipefail` in all scripts, proper quoting, no eval of user-controlled data.
...
Scope examples: `hooks`, `config`, `discord`, `validation`
