...
```bash
# Search for actionable markers in shell scripts
grep -rn "TODO\|FIXME\|HACK\|XXX\|WORKAROUND" --include="*.sh" . \
  --exclude-dir=node_modules --exclude-dir=.godot --exclude-dir=build \
  --exclude-dir=dist --exclude-dir=vendor 2>/dev/null || true
```
...
| P2 | Normal | `from-audit` issues, roadmap items |
...
|--------|-------|---------------|
| TODO | 12 | notify.sh (5), hook-handler.sh (4), tests/ (3) |
| FIXME | 3 | notify.sh (2), hook-handler.sh (1) |
| HACK | 1 | notify.sh:42 |
```
...
- **If you want to clean up:** The {N} TODOs in notify.sh suggest a refactoring pass
```
...
```bash
# Run shell test suite for quick pass/fail check
bash tests/*.sh 2>&1 | tail -20
```
...
```bash
# Check jq and curl availability and versions
jq --version
curl --version | head -1
```

- Verify jq and curl are available (core dependencies)
- Check jq version compatibility if relevant
- No package manager — dependencies are system tools
...
- Shell safety: `set -euo pipefail` in all scripts, proper quoting, input validation
...
| Dependencies | ✅ | jq and curl available and compatible |
