...
grep -rn "TODO\|FIXME\|HACK\|XXX\|WORKAROUND" --include="*.sh" . \
  ...
  --exclude-dir=dist --exclude-dir=vendor 2>/dev/null
```
...
- Verify jq and curl are available and compatible
- Check for any version-specific API usage that might break on older systems
